...
class TestDataExpiration:
    ...
    async def test_expiration_flows_through_join_to_both_sources(self):
        ...
        events_count = (
            runtime._engine.create_context().query("SELECT COUNT(*) as cnt FROM events").to_pydict()["cnt"][0]
        )
        meta_count = (
            runtime._engine.create_context().query("SELECT COUNT(*) as cnt FROM metadata").to_pydict()["cnt"][0]
        )
